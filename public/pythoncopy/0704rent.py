...
def printarea():
    ...
    #df1 = df.loc[(df['월세'] >= (renta)) & (df['전용면적'] <= (rentb))] # 월세가 아닌 경우에는 차단 
...
def printarea1():
    area = float(input("면적을 숫자로만 입력하세요 (m2) : "))
    # 다가구는 건물단위로 따지기 때문에 사이즈가 뜀
    areas = area - 66
    areab = area + 66
    df['연면적'] = df['연면적'].astype(float)
    df1 = df.loc[(df['연면적'] >= (areas)) & (df['연면적'] <= (areab))]
    print(df1)
    df1.to_excel('C:/Users/skflc/Desktop/0704result/' + filename + ' filtered.xlsx')
...
def printarea2():
    ...
    areas = area - 3.3
    areab = area + 3.3
    df['계약면적'] = df['계약면적'].astype(float)
    df1 = df.loc[(df['계약면적'] >= (areas)) & (df['계약면적'] <= (areab))]
    print(df1)
    df1.to_excel('C:/Users/skflc/Desktop/0704result/' + filename + ' filtered.xlsx')
...
def main():
    while True:
        try:
            ...
            YMD = input("연월을 입력해 주세요 예) 202206 : ")
            global ee
            global url
            global df
            global rent  # 월세
